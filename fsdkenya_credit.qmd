...
supplyside <- bind_rows(fuliza, mshwari) %>% mutate(source = "Safaricom Sustainability Report 2022 & 2023")
...
chart_df <- bind_rows(supplyside, demandside) %>% mutate(group = paste(product, source, sep = "-"))
...
ggplot(data = chart_df, 
  ...
  geom_text(aes(label = round(total_disbursed_value_b, 0)), vjust = 0, nudge_y = 15, show.legend = FALSE) +
  scale_color_manual(values = c("FinAccess" = "#F90093", "Safaricom Sustainability Report 2022 & 2023" = "#610F7F")) +
  guides(color = guide_legend(title = NULL)) + 
  labs(title = "Figure 1", 
         subtitle = "Total annual value of loan disbursements (KSh billions), supply and demand-side estimates",
         y = "KSh billions", 
         x = NULL, 
       caption = str_wrap("Notes: For comparability, estimates of the total value of disbursements have been positioned in continuous time at the end-point of the measurement period that they represent. For example, Safaricom reports figures corresponding to the cumulative value of loans disbursed during their fiscal year. Since FY2021 spans April 1st, 2020 through March 30th, 2021, figures corresponding to FY2021 are positioned at 2021.25. Estimates from FinAccess are derived from a question asking about total loans taken from specific providers during the past 12 months, so the end-point of this period is the time of data collection. For the 2021 FinAccess survey, this corresponds to August, so the estimate is positioned at 2021.7. FinAccess derived estimates for M-Shwari are not strictly comparable with Safaricom published figures for M-Shwari because FinAccess collects data on 'Loans from mobile banking' which includes M-Shwari as well as other mobile banking loan providers such as KCB M-Pesa, as such estimates from FinAccess for M-Shwari specifically are biased upwards.", 155)) + 
  theme_custom(scale_f = 1.2) + 
  theme(legend.position = "bottom", legend.direction = "horizontal", strip.text = element_text(face = "bold"))
...
```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-marketbyvolbygenderyear
#| echo: false
#| warning: false
...
plot_text <- list(
  subtitle = "Total yearly volume of loans (millions) disbursed by lender and gender, 2019 and 2021",
  title = "Figure 4",
  caption = CAPTION
) 
...
fig_bar_loans_by_slice_year(loans_data_pool, indicators, slicevar, groups, plot_text, plot_label)
...
```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-marketbyvalbygenderyear
#| echo: false
#| warning: false
#| fig-cap: "The market for loans by volume, 2019 and 2021"
...
plot_text <- list(
  subtitle = "Total yearly value of loans (millions) disbursed by lender, gender & year",
  title = "Figure 8",
  caption = CAPTION
) 
...
```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-marketbyvalbyliveyear
#| echo: false
#| warning: false
#| fig-cap: "The market for loans by volume, 2019 and 2021"
...
plot_text <- list(
  subtitle = "Total yearly value of loans (millions) disbursed by lender, borrower's main income source & year",
  title = "Figure 10",
  caption = CAPTION
) 
...
fig_bar_loans_by_slice_year(loans_data_pool %>% filter(!is.na(resp_live_group_fct)), indicators, slicevar, groups, plot_text, plot_label)
...
```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-loanprincipalbylender
#| echo: false
#| warning: false
#| fig-cap: "Median loan principal by lender, 2019 and 2021"

# Indicators used in this figure
indicators <- c("loan_principal_cd")
groups <- c("lender_agg3_str" = "Source of loan")
slicevar <- c("Year" = "year_fct")
  
plot_text <- list(
  title = "Figure 12",
   subtitle = "Median loan principal by lender, 2019 & 2021",
  yaxis = NULL,
  xaxis = "Loan principal (KSh)",
  caption =  paste(CAPTION)
) 

fig_bar_loansize_by_lender(respdebt_data_pool, indicators, slicevar, groups, plot_text)


```

```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-marketbyvolbyreasonyear
#| echo: false
#| warning: false
#| fig-cap: "The market for loans by volume, 2019 and 2021"
...
plot_text <- list(
  subtitle = "Total yearly volume of loans (millions) disbursed by lender, main reason for taking the loan & year",
  title = "Figure 13",
  caption = CAPTION
) 
...
fig_bar_loans_by_slice_year(loans_data_pool %>% filter(!is.na(loan_reason_agg_str)), indicators, slicevar, groups, plot_text, plot_label)

```

```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-marketbyvalbyreasonyear
#| echo: false
#| warning: false
#| fig-cap: "The market for loans by volume, 2019 and 2021"
...
plot_text <- list(
  subtitle = "Total yearly value of loans (billions) disbursed by lender, main reason for taking the loan & year",
  title = "Figure 14",
  caption = CAPTION
) 
...
```{r, fig.width = 12, fig.asp = 0.6}
#| label: fig-borrprevbytype_gender
#| echo: false
#| warning: false
#| fig-cap: "Prevalence of borrowing by source of loan and gender, 2019 and 2021"
...
fig_bar_borrowers_by_type_inc_bd(borrower_data_pool, indicators, slicevar, groups, plot_text)
...
pal <- c(PALETTES[[names(groups[1])]], PALETTES[[names(groups[2])]])
fig_bar_loan_reasons(loans_data, indicators, slicevar, groups, plot_text, palette = pal)
