...
# 
respdebt_data_agg1_19 <- prep_respdebt_data(borrower_data_2019, loans_data, "lender_agg1_str") # 1 row per adult-lender aggregate
respdebt_data_agg1_21 <- prep_respdebt_data(borrower_data, loans_data, "lender_agg1_str") # 1 row per adult-lender aggregate
respdebt_data_agg1_pool <- bind_rows(respdebt_data_agg1_19, respdebt_data_agg1_21)
...
# Indicators used in this figure
indicators <- c("has_recent_loan", "loan_principal_cd")
groups <- c("lender_agg0_str" = "Source of loan")
slicevar <- c("Personal monthly income" = "resp_income_quintile")

respdebt_data_agg0 <- prep_respdebt_data(borrower_data, loans_data, "lender_agg0_str") # 1 row per adult-lender aggregate
...
fig_bar_borrowers_by_type_inc(respdebt_data_agg0, indicators, slicevar, groups, plot_text)
